...
import android.os.Parcelable;
  ...
  * @since JDK1.2 
  */
public class KDTree implements Parcelable {
    ...
    */
    public void insert(double [] key, GeoPoint value) 
    ...
    * @param key key for KD-tree node
    *
    * @return GeoPoint at key, or null if not found
    *
    * @throws KeySizeException if key.length mismatches K
    */
    public GeoPoint search(double [] key) throws KeySizeException {
    ...
    * @param key key for KD-tree node
    *
    * @return GeoPoint at node nearest to key, or null on failure
    *
    * @throws KeySizeException if key.length mismatches K

    */
    public GeoPoint nearest(double [] key) throws KeySizeException {
	
	GeoPoint [] nbrs = nearest(key, 1);
	return nbrs[0];
    }
    ...
    * exceeds tree size 
    */
    public GeoPoint [] nearest(double [] key, int n) 
    ...
    */
    public GeoPoint [] range(double [] lowk, double [] uppk) 
	...
	if (lowk.length != uppk.length) {
	    ...
	    KDNode.rsearch(new HPoint(lowk), new HPoint(uppk), 
			   m_root, 0, m_K, v);
	    GeoPoint [] o = new GeoPoint[v.size()];
    ...
    public void readFromParcel(final Parcel in) {
    	m_K = in.readInt();
		m_root = in.readParcelable(KDNode.class.getClassLoader());
		m_count = in.readInt();
    }
    
    public static final Parcelable.Creator CREATOR =
        new Parcelable.Creator() {
            public KDTree createFromParcel(final Parcel in) {
                return new KDTree(in);
            }

                        @Override
                        public KDTree[] newArray(final int size) {
